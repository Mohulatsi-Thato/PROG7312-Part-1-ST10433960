...
Start the API first: cd SmartX.Api && dotnet run (leave it running).
Run the client: cd SmartX.Client && dotnet run, or press F5 on
SmartX.Client in Visual Studio (set it as the startup project).
On the landing menu, click the Sensor Data Ingestion and Telemetry
tile (the other two are intentionally disabled/greyed out per the brief).
Register a sensor, then use the buttons under "Act on Sensor":
